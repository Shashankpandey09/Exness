...
const Button = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'primary' | 'secondary' | 'ghost' | 'outline', size?: 'sm' | 'md' | 'lg' }>(
    ({ className, variant = "primary", size = "md", ...props }, ref) => {
        const variants = {
            primary: "bg-primary text-primary-foreground hover:bg-primary/90 shadow-sm shadow-blue-500/20",
            secondary: "bg-muted text-white hover:bg-muted/80",
            ghost: "hover:bg-white/5 text-neutral-300 hover:text-white",
            outline: "border border-border bg-transparent hover:bg-white/5 text-neutral-300",
        };
